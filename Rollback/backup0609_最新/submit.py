...
active_user_id.to_csv(os.path.join(os.pardir,os.pardir,'result/submit_result_')+str(datetime.now().date().month)+str(datetime.now().date().day)+str(datetime.now().time().hour)+str(datetime.now().time().minute)+'.txt', index=False, header=False)